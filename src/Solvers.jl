...
Using already calculated `displacements`, determine the forces in Newtons that each member undergoes when at equilibrium. 
Compressive forces are *negative and tensile forces are *positive*, indexes match member ids.
""" 
function solve_member_forces(setup::StaticSetup{T}, displacements::Vector{<:Vector2D}) where T
...
function solve_member_stresses(setup::StaticSetup, forces::AbstractVector)
    stresses = similar(forces)
    for (i, mid) in enumerate(member_ids(setup))
        stresses[i] = forces[i] / setup.materials[mid].area
    end
    return stresses
end
